...
int main(int argc, const char *argv[]) {
    ...
    // A section is a 32x8 piece of the map. A section has its own storage of tiles8. It also stores tiles8 of contiguous areas (i.e., a 2x2 section area)
    // Tile8 map: 64x16 (1Kb)
    // Animated tiles: 4x4x32 (0.5Kb)
    // We are doing it right: traversability, triggers and spawns.
    ...
    std::vector<std::vector<Tile8>> animatedTiles;
    {
        ...
        for (int i=0; i+7<img.rows; i+=8) {
            ...
            for (int j=0; j+7<img.cols; j+=8) {
                ...
                tile8.msx1_interlaced = P.findBestPatterns(tile8.img, true);
                tile8.count = 1;
                at.push_back(tile8);
            }
            animatedTiles.push_back(at);
            tiles8.push_back(at.front());
        }
    }
    ...
    // Find all unique 8x8 tiles.
    {
        ...
        std::cout << "Found " << tiles8.size() << " unique 8x8 tiles." << std::endl;
        if (tiles8.size()>256) { std::cerr << "Too many unique 8x8 tiles." << std::endl; exit(-1); }        
    }
    ...
    // Spawn Implementation
    {
        ...
        src << "extern const uint8_t " << (MAP_NAME + std::string("_animated")) << "[" << NUM_ANIMATED_TILES << "][" <<NUM_ANIMATED_FRAMES << "][2][16];" << std::endl;
        ...
        src << "#define MAP_TILES8 " << (MAP_NAME + "_tiles8") << std::endl;
        src << "USING_MODULE("  << (MAP_NAME + "_tiles8") << ", PAGE_C);" << std::endl;
        src << "extern const uint8_t " << (MAP_NAME + "_tiles8") << "[256][2][16];" << std::endl;
    ...
    // Spawn Animated Tiles
    {
        ...
        ofs << "const uint8_t " << (MAP_NAME + std::string("_animated")) << "[" << NUM_ANIMATED_TILES << "][" <<NUM_ANIMATED_FRAMES << "][2][16];" << std::endl;
        ofs << "const uint8_t " << (MAP_NAME + std::string("_animated")) << "[" << NUM_ANIMATED_TILES << "][" <<NUM_ANIMATED_FRAMES << "][2][16] = {" << std::endl;

        for (auto &animatedTile : animatedTiles) { ofs << "{";
            
            for (auto &tile : animatedTile) { ofs << "{";
                
                auto p = tile.msx1_interlaced;
    ...
    // Spawn tile8
    {
        std::ofstream ofs(MAP_NAME + "_tiles8" + ".c");
        ofs << "#include <stdint.h>" << std::endl;
        ofs << "const uint8_t " << (MAP_NAME + "_tiles8") << "[256][2][16];" << std::endl;
        ofs << "const uint8_t " << (MAP_NAME + "_tiles8") << "[256][2][16] = {" << std::endl;

        for (auto &tile8 : tiles8) { ofs << "{";
